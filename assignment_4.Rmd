...
author: | 
  | Lauma Ernestsone
  | l.ernestsone@students.uu.nl
  | http://www.github.com/laumarenija
date: April 2023
urlcolor: purple
linkcolor: purple
output: 
...
In the table, the mean growth is higher for countries with revolutions equal to 0 compared to countries with more than 0 revolutions, while the median growth is similar between the two groups. The variable "rgdp60" has a higher mean and median for countries with more than 0 revolutions compared to countries with revolutions equal to 0.
...
```{r}
# write t test here
t.test(GrowthSW_df$growth~GrowthSW_df$treat)
```

**Question 3.2**: What is the $p$-value of the test, and what does that mean? Write down your answer below.

The p-value of the test is 0.06871 meaning that there is 6.871% chance of obtaining that extreme of a value t=-1.8531 if the null hypothesis were true (the same mean between groups TRUE and FALSE)
...
To be able to measure average annual percentage of growth from 1960 to 1995, the rgdp60 is needed which is the GDP per capita in 1960. This rgdp60 will allow us to see whether and by how much there has been growth. The greater the rgdp60, the greater the growth.
...
```{r}
model1 <- lm(growth ~ treat, data = GrowthSW_df)
...
```{r}
library(kableExtra); library(flextable)
list(model1, model2, model3, model4) |>
  modelsummary(output = "table.docx",stars=T, gof_map = c("nobs","r.squared"))
```
